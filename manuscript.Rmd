...
knitr::opts_chunk$set(echo=FALSE, fig.height=4, fig.path='figure/')
library(broman)
```
...
```{r 01_bad_dates, fig.cap="A spreadsheet with inconsistent date formats. This spreadsheet does not adhere to our recommendations for consistency of date format."}
...
```{r 02_blank_cells, fig.width=12, fig.cap="Examples of spreadsheets that violate the 'no empty cells' recommendation. **A:** A spreadsheet where only the first of several repeated values was included. **B:** A spreadsheet with a complicated layout and some implicit column headers. For a tidy version of this data, see Figure 3."}
layout(cbind(1,2), width=c(8,10))
...
excel_fig(mat, cellwidth=110, mar=c(0.1, 0.1, 1.6, 1.1))
mtext(side=3, adj=0, "A", font=2, cex=1.2, line=0.5)
...
excel_fig(mat2, col_names=FALSE, fig_width=690, mar=c(8.4, 1.1, 1.6, 0.1))
mtext(side=3, adj=0, "B", font=2, cex=1.2, line=0.5)
```
...
```{r 03_fig2B_fixed, fig.cap="A tidy version of the data in Figure 2B.", fig.height=5}
...
```{r 04_example_rectangle, fig.height=3, fig.cap="An example spreadsheet with a rectangular layout. This layout will aid future analyses."}
set.seed(20150803)
...
```{r 05_non_rectangular, fig.height=8, fig.width=12, fig.cap="Examples of spreadsheets with non-rectangular layouts. These layouts are likely to cause problems in analysis."}
layout(rbind(1:2, 3:4), height=c(6,10))
...
mat2 <- rbind(rep("", 6),
              ...
              mat2[c(1,4),])
mat2[c(2,5,8),1] <- ""
excel_fig(mat2, col_names=FALSE, mar=c(1.1,0.1,1.6,1.1))
mtext(side=3, adj=0, "A", font=2, cex=1.2, line=0.5)
...
excel_fig(mat3, col_names=FALSE, mar=c(1.1,1.1,1.6,0.1))
mtext(side=3, adj=0, "B", font=2, cex=1.2, line=0.5)
...
excel_fig(mat4, cellwidth=c(90, 105, 95, rep(90, 5)),
          mar=c(3.8,0.1,1.6,1.1))
mtext(side=3, adj=0, "C", font=2, cex=1.2, line=0.5)
...
excel_fig(mat5, col_names=FALSE, cellwidth=c(85, 85, 105, 105, 85, 110, 110),
          mar=c(0.1,1.1,1.6,0.1))
mtext(side=3, adj=0, "D", font=2, cex=1.2, line=0.5)
```
...
```{r 06_fix_fig5D, fig.width=12, fig.height=6, fig.cap="Reorganization of Figure 5D as a pair of rectangles."}
layout(cbind(1,2), width=c(6,10))
mat5wt <- mat5[c(1,2,8,14), 1:3]
mat5wt[1,1] <- "id"
excel_fig(mat5wt, col_names=FALSE, cellwidth=c(85, 85, 105, 105),
          mar=c(19.5,0.1,1.6,1.1))
mtext(side=3, adj=0, "A", font=2, cex=1.2, line=0.5)
...
```{r 07_two_header_rows, fig.width=10, fig.cap="A spreadsheet with two header rows. It is better to have a single header row. See Figure 8 for a tidy data layout that eliminates the need for multiple header rows and repeated column headers."}
...
```{r 08_fig7_fixed, fig.height=5, fig.cap="A tidy version of the data in Figure 7."}
mat6 <- mat6[-1,]
wk <- c(4, 6, 8)
firstcol <- c(3,6,9)
...
```{r 09_data_dict, fig.height=5, fig.width=10, fig.cap="An example data dictionary."}
...
```{r 10_highlight_outlier, fig.width=10, fig.height=3, fig.cap="Highlighting in spreadsheets. **A:** A potential outlier indicated by highlighting the cell. **B:** The preferred method for indicating outliers, via an additional column."}
layout(cbind(1,2), width=c(5,6))
mat <- data.frame(id=101:107,
                  date=c("2015-06-14", "", "2015-06-18", "", "", "2015-06-20", ""),
...
```{r 11_example_rectangle, fig.height=3, fig.width=10, fig.cap="**A:** An example spreadsheet. **B:** The same data as a plain text file in CSV format."}
layout(cbind(1,2), width=c(8,5))
set.seed(20150803)
mat <- data.frame(id=101:105,
                  sex=sample(c("Male", "Female"), 5, replace=TRUE),
                  glucose=myround(runif(5, 70, 150), 1),
...
excel_fig(mat, mar=c(0.1,0.1,1.6,1.1))
mtext(side=3, adj=0, "A", font=2, cex=1.2, line=0.5)

csv <- c("id,sex,glucose,insulin,triglyc",
...
par(mar=c(6.1,1.1,1.6,0.1))
plot(0,0,type="n", xlab="", ylab="", xaxs="i", yaxs="i",
     xlim=c(0,100), ylim=c(100,0),
     xaxt="n", yaxt="n")
y <- seq(10, 90, length=length(csv))
for(i in seq(along=y))
    text(5, y[i], csv[i], adj=c(0, 0.5), family="mono")
mtext(side=3, adj=0, "B", font=2, cex=1.2, line=0.5)
```
